    ...
    this.allServicesService.getTransferStory().subscribe((data: any) => {
      ...
      for (var i = 0; i < propertyValues1.length; i++) {
        ...
        for (var j = 0; j < employeeDetail.length; j++) {
          if (
            employeeDetail[j].employee_event_name.name ==
            EmployeeTransferHistories.Department
          ) {
            this.isData = true;
            this.year.department = employeeDetail[j].new_label;
          } else if (
            employeeDetail[j].employee_event_name.name ==
            EmployeeTransferHistories.Designation
          ) {
            this.isData = true;
            employeeDetail[j].old_label
              ? this.designations.push(employeeDetail[j].old_label)
              : null;
            employeeDetail[j].new_label
              ? this.designations.push(employeeDetail[j].new_label)
              : null;
            this.year.designations = this.designations;
          } else if (
            employeeDetail[j].employee_event_name.name ==
            EmployeeTransferHistories.Office
          ) {
            this.isData = true;
            this.year.office = employeeDetail[j].new_label;
          } else if (
            employeeDetail[j].employee_event_name.name ==
            EmployeeTransferHistories.Location
          ) {
            this.isData = true;
